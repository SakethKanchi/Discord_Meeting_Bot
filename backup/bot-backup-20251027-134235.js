...
async function processSegmentChronologically(segmentKey) {
    ...
    return new Promise((resolve, reject) => {
        ...
        // Calculate timing offsets for each speaker
        const timingOffsets = fileData.map(({ timestamp }, index) => {
            const startTime = fileData[0].timestamp; // First speaker's start time
            const offsetMs = timestamp - startTime;
            const offsetSeconds = offsetMs / 1000;
            return offsetSeconds;
        });

        console.log(`📊 Timing offsets: ${timingOffsets.map((offset, i) => `${fileData[i].username}: +${offset.toFixed(2)}s`).join(', ')}`);
        ...
        // Build filter complex for proper mixing with timing
        let filterComplex = '';
        const mixInputs = [];
        ...
        for (let i = 0; i < wavFiles.length; i++) {
            ...
            if (offset === 0) {
                ...
                // Other speakers start at their timestamp offset
                mixInputs.push(`[${i}:a]adelay=${offset * 1000}|${offset * 1000}[delayed${i}]`);
                filterComplex += `[${i}:a]adelay=${offset * 1000}|${offset * 1000}[delayed${i}];`;
            }
        }

        // Mix all delayed streams together
        const delayedInputs = fileData.map((_, i) => timingOffsets[i] === 0 ? `[${i}:a]` : `[delayed${i}]`);
        filterComplex += `${delayedInputs.join('')}amix=inputs=${wavFiles.length}:duration=longest[out]`;
...
async function startRecording(connection, guild, channelId) {
    ...
    // Set up voice activity detection for speaking start
    connection.receiver.speaking.on('start', (userId) => {
        ...
        // Capture current segment counter for this stream
        const currentSegment = recordingState.segmentCounter;
        const userOutputStream = fs.createWriteStream(
            path.join(PCM_FOLDER, `${recordingState.channelName}_${recordingState.timestamp}_segment_${currentSegment}_${user.displayName}_${Date.now()}.pcm`)
        );
        ...
        const endHandler = () => {
            ...
            // Manually destroy the audio stream and unsubscribe
            audioStream.destroy();
            connection.receiver.unsubscribe(userId);
        ...
        connection.receiver.speaking.on('end', (endUserId) => {
            if (endUserId === userId) {
                endHandler();
            }
        });
...
async function stopRecording() {
    ...
    try {
        ...
        if (meetingFiles.length > 0) {
            ...
            const summary = await summarizeTranscript(transcript, attendees, recordingState.channelName);
            ...
            await saveSummaryLocally(summary, attendees, recordingState.channelName);
            ...
            meetingFiles.forEach(file => {
                try {
                    fs.unlinkSync(path.join(PCM_FOLDER, file));
